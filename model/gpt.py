...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        for _ in range(num_blocks):
            blocks.append(self.TransformerBlock(model_dim,num_heads))
        self.gpt_model=nn.Sequential(*blocks)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        position=torch.arange(context.shape[1])
        positional_embed=self.positional_embedding(position)
        embedded=token_embed+positional_embed
        embedded=self.gpt_model(embedded)
        embedded_norm=self.final_norm(embedded)
        projection=self.vocab_projection(embedded_norm)
